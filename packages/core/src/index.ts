...
const parseDate = (date: LocalDate): Date => {
  ...
  if (Number.isNaN(parsed.valueOf()) || parsed.toISOString().slice(0, 10) !== date) throw new RangeError(`Invalid LocalDate: ${date}`);
  return parsed;
};
const addDays = (date: LocalDate, count: number): LocalDate => new Date(parseDate(date).valueOf() + count * DAY).toISOString().slice(0, 10);
const diffDays = (left: LocalDate, right: LocalDate): number => Math.round((parseDate(left).valueOf() - parseDate(right).valueOf()) / DAY);
...
export function localDateFor(instant: Date, tz: string, dayStartHour: number): LocalDate {
  if (!Number.isInteger(dayStartHour) || dayStartHour < 0 || dayStartHour > 23) throw new RangeError("dayStartHour must be 0..23");
...
export function scheduleAt(versions: ScheduleVersion[], date: LocalDate): Schedule {
  parseDate(date);
  const active = versions.filter(v => v.validFrom <= date).sort((a, b) => b.validFrom.localeCompare(a.validFrom))[0];
...
export function isDue(schedule: Schedule, date: LocalDate): boolean {
  parseDate(date);
  if (schedule.kind === "daily" || schedule.kind === "times_per_week" || schedule.kind === "times_per_month") return true;
  if (schedule.kind === "days_of_week") return schedule.days.includes(isoWeekday(date));
  if (!Number.isInteger(schedule.every) || schedule.every <= 0) throw new RangeError("Interval must be positive");
  return ((diffDays(date, schedule.anchor) % schedule.every) + schedule.every) % schedule.every === 0;
}

type StreakParams = { versions: ScheduleVersion[]; entries: Entry[]; today: LocalDate; startedOn: LocalDate };
...
function fixedStreak(params: StreakParams): Streak {
  ...
  for (const outcome of outcomes) {
    if (outcome === "done") { run++; best = Math.max(best, run); }
...
const weekKey = (date: LocalDate): string => {
  const d = parseDate(date); d.setUTCDate(d.getUTCDate() + 4 - (d.getUTCDay() || 7));
...
function periodStreak(params: StreakParams, unit: "week" | "month"): Streak {
  ...
  for (let date = params.startedOn; date <= params.today; date = addDays(date, 1)) {
    const key = keyFor(date); if (!periods.includes(key)) periods.push(key);
  }
  const todayKey = keyFor(params.today);
  const outcomes = periods.filter(key => key !== todayKey).map(key => {
    const dates = params.entries.filter(e => keyFor(e.localDate) === key && e.status === "done");
    const representative = unit === "week" ? params.entries.find(e => keyFor(e.localDate) === key)?.localDate : `${key}-01`;
    const date = representative ?? params.startedOn;
    const schedule = scheduleAt(params.versions, date);
    const target = schedule.kind === "times_per_week" || schedule.kind === "times_per_month" ? schedule.target : 1;
    return dates.length >= target;
  });
  let run = 0, best = 0;
  for (const complete of outcomes) { run = complete ? run + 1 : 0; best = Math.max(best, run); }
...
export function completionRate(params: StreakParams): number | null {
  ...
  for (let date = params.startedOn; date <= params.today; date = addDays(date, 1)) {
    ...
    due++; if (status === "done") done++;
  }
  return due === 0 ? null : done / due;
}

const maskIncludes = (mask: number[] | number, day: number): boolean => Array.isArray(mask) ? mask.includes(day) : (mask & (1 << (day - 1))) !== 0;
...
export function nextFireAt(reminder: Reminder, tz: string, now: Date): Date {
  ...
  for (let offset = 0; offset < 8; offset++) {
    ...
    for (let stamp = estimate - 15 * 3_600_000; stamp <= estimate + 15 * 3_600_000; stamp += 60_000) {
      const candidate = new Date(stamp); const p = zonedParts(candidate, tz);
      if (p.year !== y || p.month !== m || p.day !== d) continue;
      if (p.hour === hour && p.minute === minute && p.second === second && candidate > now) return candidate;
      const localMinutes = (p.hour ?? 0) * 60 + (p.minute ?? 0);
      if (!firstAfterGap && localMinutes > hour * 60 + minute && candidate > now) firstAfterGap = candidate;
    }

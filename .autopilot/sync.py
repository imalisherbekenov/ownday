...
def serves_this_run(port, state):
    """Наш ли сервер на этом порту — по содержимому, а не по ps.
    В Git Bash на Windows `ps -o command=` про native python.exe не знает ничего,
    поэтому is_ours() там всегда ложно и каждый вызов поднимал новый сервер."""
    want = state.get("dir") or state.get("slug")
    if not want:
        return False
    try:
        with urllib.request.urlopen("http://127.0.0.1:%d/state.js" % port, timeout=2) as r:
            return want in r.read(65536).decode("utf-8", "replace")
    except Exception:
        return False
...
def serve(state):
    ...
    port, pid = recorded()
    if port and http_ok(port) and (serves_this_run(port, state)
                                   or not pid or is_ours(cmdline(pid))):

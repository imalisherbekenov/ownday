...
describe("StreakPill", () => {
  it("does not render zero", () => {
    const { container } = render(<StreakPill streak={0} />);
...
describe("HabitRow", () => {
  ...
  it("places a counter below a full-width, shrink-safe title", () => {
    render(
      <HabitRow
        {...base}
        type="counter"
    ...
    const content = screen.getByTestId("habit-content");
    const title = screen.getByTestId("habit-title-container");
    const counterRow = screen.getByTestId("counter-row");

    expect(content).toHaveClass("min-w-0", "flex-1");
    expect(title).toHaveClass("min-w-0");
    expect(title.querySelector("span")).not.toHaveClass("truncate");
    expect(counterRow.parentElement).toBe(content);
    expect(
      counterRow.compareDocumentPosition(title) & Node.DOCUMENT_POSITION_PRECEDING,
    ).toBeTruthy();
  });
});
describe("Heatmap", () => {
  it("maps all five intensity levels", () => {
    const points = ([0, 1, 2, 3, 4] as const).map((intensity, i) => ({
      date: `2026-01-0${i + 1}`,
      intensity,
    }));
